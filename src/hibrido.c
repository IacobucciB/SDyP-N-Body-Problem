...
cuerpo_t *cuerpos_local; // Bloque de cuerpos para cada proceso MPI 
double *fuerzas_localX, *fuerzas_localY, *fuerzas_localZ; // Fuerzas acumuladas para cada proceso MPI

// Buffers temporales para comunicación entre workers
...
// ----- PTHREADS -----
int T_PTHREADS; // Total de hilos 

void *calcularFuerzasInternasThreads(void *arg);
void *calcularFuerzasEntreBloquesThreads(void *arg);
void *moverCuerposThreads(void *arg); 
...
// Funciones para MPI
void calcularFuerzasInternas(cuerpo_t *bloque_cuerpos, double *F1_totalX, double *F1_totalY, double *F1_totalZ, int idW_Thread);
void calcularFuerzasEntreBloques(cuerpo_t *bloque_cuerpos_1, cuerpo_t *bloque_cuerpos_2, double *F1_totalX, double *F1_totalY, double *F1_totalZ, double *F2_totalX, double *F2_totalY, double *F2_totalZ, int idW_Thread);
void moverCuerpos(cuerpo_t *cuerpos_totales, double *F_totalX, double *F_totalY, double *F_totalZ, int blocksize, int idW_Thread);
void Worker(void);
...
void Worker(void)
{
    ...
    // Creamos vectores para threads y sus IDs
    pthread_t threads[T_PTHREADS];
    int thread_ids[T_PTHREADS];

    // ====================================================================
    // BUCLE PRINCIPAL DE SIMULACIÓN (PASOS DE TIEMPO)
    // ====================================================================

    // Solo el worker 0 (cero) inicia el tiempo de simulación
    if(idW_MPI == 0)
    {
        tIni = dwalltime();
    }
    ...
    for (int paso = 0; paso < pasos; paso++)
    {
        ...
        for(int i = 0; i < T_PTHREADS; i++) {
            thread_ids[i] = i;
            pthread_create(&threads[i], NULL, calcularFuerzasInternasThreads, (void *)&thread_ids[i]);
        }
        ...
        { 
            ...
            for(int i = 0; i < T_PTHREADS; i++) {
                thread_ids[i] = i;
                pthread_create(&threads[i], NULL, calcularFuerzasEntreBloquesThreads, (void *)&thread_ids[i]);
            }
        ...
        for(int i = 0; i < T_PTHREADS; i++) {
            thread_ids[i] = i;
            pthread_create(&threads[i], NULL, moverCuerposThreads, (void *)&thread_ids[i]);
        }
...
void *calcularFuerzasInternasThreads(void *arg)
{
    int idW_Thread = *((int *) arg);
    calcularFuerzasInternas(cuerpos_local, fuerzas_localX, fuerzas_localY, fuerzas_localZ, idW_Thread);
}

void *calcularFuerzasEntreBloquesThreads(void *arg)
{
    int idW_Thread = *((int *) arg);
    calcularFuerzasEntreBloques(cuerpos_local, cuerpos_temp, fuerzas_localX, fuerzas_localY, fuerzas_localZ, fuerzas_tempX, fuerzas_tempY, fuerzas_tempZ, idW_Thread);
}

void *moverCuerposThreads(void *arg)
{
    int idW_Thread = *((int *) arg);
    moverCuerpos(cuerpos_local, fuerzas_localX, fuerzas_localY, fuerzas_localZ, blockSize, idW_Thread);
}
...
void calcularFuerzasInternas(cuerpo_t *bloque_cuerpos, double *F1_totalX, double *F1_totalY, double *F1_totalZ, int idW_Thread)
{
    ...
    int slice_thread = blockSize / T_PTHREADS;
    int ini_thread = idW_Thread * slice_thread;
    int lim_thread = ini_thread + slice_thread;

    for (cuerpo1 = ini_thread; cuerpo1 < lim_thread - 1; cuerpo1++)
    {
        for (cuerpo2 = cuerpo1 + 1; cuerpo2 < lim_thread; cuerpo2++)
        {
...
void calcularFuerzasEntreBloques(cuerpo_t *bloque_cuerpos_1, cuerpo_t *bloque_cuerpos_2, double *F1_totalX, double *F1_totalY, double *F1_totalZ, double *F2_totalX, double *F2_totalY, double *F2_totalZ, int idW_Thread)
{
    ...
    for (cuerpo1 = ini_thread; cuerpo1 < lim_thread; cuerpo1++)
    {
        for (cuerpo2 = ini_thread; cuerpo2 < lim_thread; cuerpo2++)
        {
...
void moverCuerpos(cuerpo_t *cuerpos_totales, double *F_totalX, double *F_totalY, double *F_totalZ, int blocksize, int idW_Thread)
{
    ...
    for (cuerpo = ini_thread; cuerpo < lim_thread; cuerpo++)
    {
